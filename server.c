...
/* response is the actual response string from the handle_it function */
static int
read_command(int s, char *buffer, char *response) 
{
  ...
  for (;;) {
    ...
    *c = '\0';
    ...
    /* No format specifiers specified, a buffer including format specifiers to read the 
    stack can be provided (each argument of the format specifiers will be taken on the stack) */
...
/* response is the answer string from the handle_it function, of length 128 bytes */
static void
execute_command(int s, unsigned int x, char *buffer, char *response)
{
  ...
  /* Make copy */
  /* strcpy is vulnerable since buffer size if 256 bytes and response(answer) size is 128 bytes */
  strcpy(response, buffer);
}

/* Despite the declaration order x is above answer in the stack */
static void
handle_it(int s)
{
  /* answer's length 128, others length 256 */
  char answer[ALENGTH];
